...
5. **Проверка зашифрованных данных (📸 Обязательный Скриншот №2):**
...
# Отчет по лабораторной работе: Шифрование данных и локальная безопасность (Debian 13)
...
### Подробный ход выполнения и команды:
...
3. **Фиксация исходного состояния данных (📸 Обязательный Скриншот №1):**
   Переходим в созданный домашний каталог пользователя, создаем там текстовый проверочный файл и читаем его содержимое:
   ```bash
   cd /home/cryptouser
   echo "My unencrypted secret text 2026" > open_test.txt
   ...
   cat open_test.txt
   ```
   *Пояснение для новичка:* На этом этапе (до миграции) данные лежат на жестком диске в открытом виде. Любой пользователь с правами администратора (`root`) может зайти в папку и прочитать содержимое файла `open_test.txt`. На Скриншоте №1 фиксируется этот факт.
...
5. **Проверка зашифрованных данных (📸 Обязательный Скриншот №2):**
   ...
   ls -la /home/cryptouser
   ```
   *Пояснение для новичка:* На Скриншоте №2 будет видно, что привычного файла `open_test.txt` в папке больше нет. Вместо него система отображает только две скрытые служебные папки: `.ecryptfs` (где хранятся крипто-ключи) и `.Private` (где лежат зашифрованные файлы, имена которых превратились в случайный набор символов). Данные успешно заперты. Они автоматически расшифруются в памяти системы только в момент легитимного ввода пароля самим пользователем `cryptouser` при входе.

---

## ЗАДАНИЕ 2: Шифрование разделов с помощью LUKS

**Цель задания:** Установить поддержку стандарта LUKS и создать изолированный зашифрованный раздел объемом ровно 100 МБ.

> **Пояснение для новичка по методу выполнения:** Чтобы случайно не повредить реальную разметку жесткого диска вашей виртуальной машины, применяется полностью безопасный метод *loop-устройств*. Мы создаем обычный файл размером 100 МБ и заставляем ядро Linux относиться к нему как к настоящей подключенной физической флешке.

### Поэтапный ход выполнения и команды (под `root`):
...
3. **Инициализация LUKS-контейнера (📸 Поэтапный Скриншот №1):**
   Размечаем созданный виртуальный диск под шифрованный контейнер LUKS:
   ```bash
   /sbin/cryptsetup luksFormat /dev/loop0
   ```
   *Важные действия на этом этапе:* Система выдаст предупреждение о безвозвратном затирании данных. Вводим строго заглавными буквами слово **`YES`**, после чего дважды указываем надежный пароль (например, `luks_password123`). На Скриншоте №1 фиксируется процесс успешного создания контейнера.

4. **Открытие и форматирование диска (📸 Поэтапный Скриншот №2):**
   ...
   ```
   На Скриншоте №2 фиксируются технические строки успешного форматирования раздела и создания суперблоков файловой системы.

5. **Монтирование и проверка статуса (📸 Поэтапный Скриншот №3):**
   ...
   ```
   *Пояснение для новичка:* На Скриншоте №3 отображается финальный статус устройства. В выводе терминала четко видно, что крипто-текст `/dev/mapper/secure_disk` находится в статусе `active`, использует банковский алгоритм шифрования `aes-xts-plain64` и имеет объем около 100 МБ. 

По окончании демонстрации диск был корректно размонтирован и закрыт в памяти командами `umount /mnt/my_secure_data` и `/sbin/cryptsetup close secure_disk`.
